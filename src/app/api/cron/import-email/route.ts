...
export async function GET(req: NextRequest) {
    try {
        ...
        const type = searchParams.get('type') || 'all'; // 'rdn', 'futures', or 'all'

        console.log(`[Cron] Starting Automated Email Import (Mode: ${type})...`);
            ...
            if (messagesF.length > 0) {
                ...
                if (attachmentId) {
                    ...
                    if (csvData) {
                        ...
                        results.push(`Futures: ${result.processed} records. ${result.details || ''}`);
                    } else {
